 ...
 * 
 * 인터프리터란 고수준 언어로 작성된 프로그램을 실행하는 프로그램이다.
 * 컴파일러와의 차이점: 컴파일러는 전체 소스 코드를 기계어로 변환한 후 실행하지만, 인터프리터는 소스 코드를 한 줄씩 읽어 실행한다.
 * 이로 인해 인터프리터는 유연성이 높고(any를 사용해 모든 타입을 처리할 수 있음) 다만 컴파일러에 비해 실행 속도가 느리다.
*/
...
// ex) if (true) { var a = 10; }
// 지역변수 a는 if문의 블록 내부에서만 유효하다.
// if문의 블록 내부에서만 유효하므로, if문의 블록을 나타내는 지역 스코프에 저장된다.
